...
void MyRobot::turnLeft(float duration){
  ...
  myDelay(duration * MILLISECONDS_IN_SECOND);
  Halt();
}
...
void MyRobot::myDelay(float duration_ms){
  ...
  while((currentMillis - startMillis) < duration_ms){
    //check time until we've waited the specified time.
    //update time
